...
export function findLatestMachineMessage(comments = []) {
  for (let index = comments.length - 1; index >= 0; index -= 1) {
    const comment = comments[index];
    const body = String(comment?.body || "");
    ...
    return {
      index,
      comment,
      source: response ? "ai-response" : "automation",
      response,
      metadata: extractMetadata(body),
      classification: classifyComment(body),
    };
...
export function renderMachineMessageMarkdown({ issue = {}, machineMessage = null }) {
  if (!machineMessage?.comment) {
    return [
      "# Last Machine Message",
      `- thread: ${issue?.number ?? "unknown"}`,
      `- workspace: ${resolveWorkspaceName(issue)}`,
      "",
      "_No machine message found._",
    ].join("\n");
  }
  return renderMessageBlock({
    heading: "# Last Machine Message",
    source: machineMessage.source || "machine",
